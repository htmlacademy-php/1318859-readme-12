                ...
                <div class="post-details__main-block post post--details">
                    ...
                    <div class="post__indicators">
                        <div class="post__buttons">
                            ...
                            <a class="post__indicator post__indicator--repost button <?= (isset($reposted_post_ids_by_session_user)
                                && in_array($post['id'], $reposted_post_ids_by_session_user))
                                ? 'post__indicator--repost-active' : '' ?>"
                               href="post.php?id=<?= $id ?? '' ?>&reposted"
                               title="Репост">
                                <svg class="post__indicator-icon <?= (isset($reposted_post_ids_by_session_user)
                                    && in_array($post['id'], $reposted_post_ids_by_session_user))
                                    ? 'post__indicator-icon--repost-active' : '' ?>" width="19" height="17">
                        ...
                        <span class="post__view"><?= $views_count ?? '0' ?> <?= get_noun_plural_form($views_count,
                        ...
                        <form class="comments__form form" action="post.php?id=<?= $id ?? '' ?>" method="post">
                            ...
                            <div class="form__input-section <?php if (!empty($errors[$form['name']])): ?>form__input-section--error<?php endif; ?>">
                                <textarea class="comments__textarea form__textarea form__input"
                                          ...
                                          <?php if ($count_of_shown_post_comments
                                          === 0): ?>id="last_comment"<?php endif; ?>><?= $_POST['comment'] ??
                                    '' ?></textarea>
                            ...
                            <div class="comments__list-wrapper">
                                <ul class="comments__list">
                                    <?php for ($i = 0; $i < $count_of_shown_post_comments; $i++): ?>
                                        <li class="comments__item user" <?php if ($i === $count_of_shown_post_comments
                                        - 1): ?>id="last_comment"<?php endif; ?>>
                ...
                <div class="post-details__user user">
                    ...
                    <div class="post-details__rating user__rating">
                        <p class="post-details__rating-item user__rating-item user__rating-item--subscribers">
                            <span class="post-details__rating-amount user__rating-amount"><?= $amount_of_user_followers
                                ?? '0' ?></span>
                        ...
                        <p class="post-details__rating-item user__rating-item user__rating-item--publications">
                            <span class="post-details__rating-amount user__rating-amount"><?= $amount_of_user_posts ??
                                '0' ?></span>
                    ...
                    <?php if (!$self_page): ?>
                        <div class="post-details__user-buttons user__buttons">
                            <?php if ($subscribe): ?>
                                <a class="user__button user__button--subscription button button--quartz" href="profile.php?id=<?= $post['user_id']
                                ?? '' ?>&unsubscribed">
                                    Отписаться
                                </a>
                                <a class="user__button user__button--writing button button--green" href="message.php?id=<?= $post['user_id']
                                ?? '' ?>">Сообщение</a>
                            <?php else: ?>
                                <a class="user__button user__button--subscription button button--main" href="profile.php?id=<?= $post['user_id']
                                ?? '' ?>&subscribed">
                                    Подписаться
                                </a>
                            <?php endif; ?>
