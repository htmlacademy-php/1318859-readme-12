            ...
            <div class="profile__user user container">
                ...
                <div class="profile__rating user__rating">
                    <p class="profile__rating-item user__rating-item user__rating-item--publications">
                        ...
                        <span class="profile__rating-text user__rating-text"><?= get_noun_plural_form($amount_of_user_posts, 'публикация', 'публикации', 'публикаций') ?></span>
                    </p>
                    <p class="profile__rating-item user__rating-item user__rating-item--subscribers">
                        <span class="user__rating-amount"><?= $amount_of_user_followers ?? '0' ?></span>
                        <span class="profile__rating-text user__rating-text"><?= get_noun_plural_form($amount_of_user_followers, 'подписчик', 'подписчика', 'подписчиков') ?></span>
                    </p>
                </div>
                    ...
                    <ul class="profile__tabs-list filters__list tabs__list">
                        <?php foreach ($tabs as $type => $name): ?>
                            <li class="profile__tabs-item filters__item">
                                <a class="profile__tabs-link filters__button <?php if ($type === $current_tab): ?>filters__button--active tabs__item--active<?php endif; ?> tabs__item button" href="/profile.php?id=<?= $user['id'] ?? '' ?>&tab=<?= $type ?? '' ?>"><?= $name ?? '' ?></a>
                            </li>
                        <?php endforeach; ?>
                    </ul>
                </div>
                        ...
                        <section class="profile__subscriptions tabs__content tabs__content--active">
                            ...
                            <?php if (!empty($followers_of_user)): ?>
                                <ul class="profile__subscriptions-list">
                                    <?php foreach (
                                        $followers_of_user as $follower
                                    ): ?>
                                        <li class="post-mini post-mini--photo post user">
                                            <div class="post-mini__user-info user__info">
                                                <div class="post-mini__avatar user__avatar">
                                                    <a class="user__avatar-link" href="profile.php?id=<?= $follower['id'] ?? '' ?>">
                                                        <img class="post-mini__picture user__picture" src="<?= $follower['avatar'] ?? '' ?>" alt="Аватар пользователя">
                                                    </a>
                                                </div>
                                                <div class="post-mini__name-wrapper user__name-wrapper">
                                            ...
                                            <div class="post-mini__user-buttons user__buttons">
                                                <?php if ($follower['id'] !== intval($_SESSION['user']['id'])): ?>
                                                    <?php if ($follower['subscribed_by_session_user']): ?>
                                                        <a class="post-mini__user-button user__button user__button--subscription button button--quartz" href="profile.php?id=<?= $follower['id'] ?? '' ?>&unsubscribed">
                                                            Отписаться
                                                        </a>
                                                    <?php else: ?>
                                                    ...
                                                    <?php endif; ?>
                                                <?php else: ?>
                                                    <a class="post-mini__user-button user__button user__button--subscription button button--main" href="profile.php?id=<?= $follower['id'] ?? '' ?>">
                                                        На свою страницу
                                                    </a>
                                                <?php endif; ?>
                                            </div>
                                        ...
                                        <footer class="post__footer">
                                            <div class="post__indicators">
                                                <div class="post__buttons">
                                                    ...
                                                    <a class="post__indicator post__indicator--repost button <?= (isset($reposted_post_ids_by_session_user) && in_array($post['id'], $reposted_post_ids_by_session_user)) ? 'post__indicator--repost-active' : '' ?>"
                                                       href="profile.php?id=<?= $user['id'] ?? '' ?>&reposted_post_id=<?= $post['id'] ?? '' ?>" title="Репост">
                                                        <svg class="post__indicator-icon <?= (isset($reposted_post_ids_by_session_user) && in_array($post['id'], $reposted_post_ids_by_session_user)) ? 'post__indicator-icon--repost-active' : '' ?>" width="19" height="17">
