                ...
                <ul class="popular__filters-list filters__list">
                    ...
                    <?php foreach ($types as $type) : ?>
                        <li class="popular__filters-item filters__item">
                            ...
                            <?php if (isset($type['id']) && intval($type_id) === $type['id']) : ?>
                            filters__button--active
                            <?php endif; ?>
                            button" href="popular.php?type_id=<?= $type['id'] ?? '' ?>">
                                <span class="visually-hidden">
                                <?php if (isset($type['title']) && $type['title'] === 'Картинка') : ?>
                                    Фото
                                <?php else : ?>
                                    <?= $type['title'] ?? '' ?>
                                <?php endif; ?>
        ...
        <div class="popular__posts">
            <?php if (!empty($posts)) : ?>
                <?php foreach ($posts as $post) : ?>
                    <article class="popular__post <?= (isset($post['class_name']) && $post['class_name']) ? "post-"
                        . $post['class_name'] : "" ?>">
                                ...
                                <?php elseif ($post['class_name'] === 'video') : ?>
                                    <div class="post-video__block">
                                        <div class="post-video__preview">
                                            <?= embed_youtube_cover($post['video'] ?? '', 360, 188); ?>
                                        </div>
                                        <a href="post.php?id=<?= $post['id'] ??
                                ...
                                '' ?>" title="Автор">
                                    ...
                                    <div class="post__info">
                                        ...
                                        <time class="post__time"
                                              datetime="<?= date_format(date_create($post['dt_add']
                                                  ?? ''), 'Y-m-d'); ?>"
                            ...
                            <div class="post__indicators">
                                <div class="post__buttons">
                                    ...
                                    <a class="post__indicator post__indicator--comments button"
                                       href="post.php?id=<?= $post['id'] ?? '' ?>#last_comment" title="Комментарии">
                                        <svg class="post__indicator-icon" width="19" height="17">
                                            <use xlink:href="#icon-comment"></use>
                                        </svg>
                                        <span><?= (isset($post['id']))
                                                ? count_comments_of_post($con, $post['id']) : '' ?></span>
        ...
        <div class="popular__page-links">
            <?php if ($page_number > 1) : ?>
                <a class="popular__page-link popular__page-link--prev button button--gray"
                   href="popular.php?page_num=<?= ($page_number - 1) ?><?= (isset($_GET['sort'])) ? '&sort='
                       . $_GET['sort'] : '' ?><?= (isset($_GET['type_id']))
                       ? '&type_id=' . $_GET['type_id'] : '' ?>">Предыдущая
                    страница</a>
            <?php else : ?>
                <a class="popular__page-link popular__page-link--hidden popular__page-link--prev button button--gray"
                   href="popular.php?page_num=<?= $page_number ?><?= (isset($_GET['sort'])) ? '&sort=' . $_GET['sort']
                       : '' ?><?= (isset($_GET['type_id'])) ? '&type_id=' . $_GET['type_id'] : '' ?>">Предыдущая
                    страница</a>
            <?php endif; ?>
            <?php if ($page_number < $max_page) : ?>
                <a class="popular__page-link popular__page-link--next button button--gray"
                   href="popular.php?page_num=<?= ($page_number + 1) ?><?= (isset($_GET['sort'])) ? '&sort='
                       . $_GET['sort'] : '' ?><?= (isset($_GET['type_id']))
                       ? '&type_id=' . $_GET['type_id'] : '' ?>">Следующая
                    страница</a>
            <?php endif; ?>
        </div>
    </div>
